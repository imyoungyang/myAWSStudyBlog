...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope('deepfm_model', reuse=tf.AUTO_REUSE):
        ...
        #liangaws:利用TowerOptimizer对optimizer做wrapper
...
def main(_):
    ...
    #liangaws:测试实例多个GPU或者CPU来用replicate_model_fn对原model_fn做wrapper
    
    DeepFM = tf.estimator.Estimator(model_fn=tf.contrib.estimator.replicate_model_fn(model_fn), model_dir=FLAGS.model_dir, params=model_params)
   
    
    
    #liangaws：当一个训练实例是多GPU或者多CPU时，为了利用多GPU或者多CPU，设置batch size与GPU和CPU数量成比例。
    
    if FLAGS.num_GPUs > 0:
        batch_size = FLAGS.batch_size * FLAGS.num_GPUs
    elif FLAGS.num_CPUs > 2:  #如果是分布式训练，parameter server会使用一个VCPU 
